...
module TableGen
  module ApplicationHelper
    ...
    def empty_state(**args)
      render TableGen::EmptyStateComponent.new(**args)
    end

    def a_button(**args, &block)
      render TableGen::Shared::ButtonComponent.new(is_link: false, **args) do
        capture(&block) if block.present?
      end
    end

    def a_link(path = nil, **args, &block)
      render TableGen::Shared::ButtonComponent.new(path, is_link: true, **args) do
        capture(&block) if block.present?
      end
    end
    ...
    def svg(file_name, **args)
      ...
      with_asset_finder(::TableGen::SvgFinder) do
    ...
    def mount_path
      TableGen::Engine.routes.find_script_name(params.permit!.to_h.symbolize_keys)
    end
