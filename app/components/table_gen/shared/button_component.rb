...
module TableGen
  module Shared
    class ButtonComponent < ViewComponent::Base
      ...
      def initialize(path = nil, size: :md, style: :outline, color: :gray, icon: nil, icon_class: '', is_link: false,
                     rounded: true, compact: false, **args)
        # Main settings
        super
        @size = size
        @style = style
      ...
      def args
        if @args[:loading]
          ...
          @args[:"data-loading-button-confirmation-message-value"] = @args.delete(:confirm) if @args[:confirm]
        end
      ...
      def output_button
        if args[:method].present? || args.dig(:data, :turbo_method).present?
