...
ASSET_FILE = ARGV[0] || "app/assets/builds/table_gen.tailwind.css"
...
if Gem.loaded_specs.key? TAILWINDCSS_RAILS
  ...
  AVO_TAILWIND_COMPILE_COMMAND = "#{RbConfig.ruby} #{Pathname.new(GEM_PATH)}/exe/tailwindcss -i '#{Rails.root.join("app/assets/stylesheets/table_gen.tailwind.css")}' -o '#{Rails.root.join(ASSET_FILE)}' -c '#{Rails.root.join("config/tailwind.config.js")}' --minify"

  namespace "table_gen:tailwindcss" do
